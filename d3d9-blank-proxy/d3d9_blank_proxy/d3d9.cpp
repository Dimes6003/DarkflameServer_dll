...
DWORD WINAPI DeferredStartup(LPVOID lpParameter);
HWND FindWindowInCurrentProcess(DWORD targetPid);
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    ...
    case DLL_PROCESS_ATTACH:
    {
        ...
        if (NULL == d3d9.dll)
        {
            Log("Failed to load original d3d9.dll");
            MessageBox(0, "Cannot load original d3d9.dll library", "Proxy", MB_ICONERROR);
            ExitProcess(0);
        }
        Log("Loaded original d3d9.dll from: %s", path);
...
{
    Log("Waiting for main game window owned by this process...");

    DWORD currentPid = GetCurrentProcessId();
    while ((g_hMainWindow = FindWindowInCurrentProcess(currentPid)) == NULL)
    {
        Sleep(100);  // Check every 100ms
    }

    Log("Main game window found (and verified by PID).");
    RunMainCode();
    return 0;
}
...
BOOL CALLBACK EnumWindowsProc(HWND hwnd, LPARAM lParam)
{
    ...
    if (pid != pData->targetPid)
        return TRUE; // Not our process, continue enumeration
